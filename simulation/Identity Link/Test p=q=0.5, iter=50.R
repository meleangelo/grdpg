...
phat=result$phat
qhat=result$qhat
betaerror=betahat-beta
perror=phat-latent[,1]
qerror=qhat-latent[,2]
for (i in seq(2,50)){
  seed <- 2019+i
  ...
  phat=result$phat
  qhat=result$qhat
  betaerror=c(betaerror,betahat-beta)
  perror=c(perror,phat-latent[,1])
  qerror=c(qerror,qhat-latent[,2])
}
...
pp1 <- pp1 + labs(title = 'p=q=0.5, True beta=0.15, n=2000', x = 'Iteration', y = 'beta error')


dat=data.frame(perror)
pp2 <- ggplot(dat, aes(x=seq(50), y=perror)) + geom_line() + geom_point()
pp2 <- pp2 + labs(title = 'p=q=0.5, True beta=0.15, n=2000', x = 'Iteration', y = 'p error')


dat=data.frame(qerror)
pp3 <- ggplot(dat, aes(x=seq(50), y=qerror)) + geom_line() + geom_point()
pp3 <- pp3 + labs(title = 'p=q=0.5, True beta=0.15, n=2000', x = 'Iteration', y = 'q error')
