...
for (pi_cov_ in seq(0.1+bandwidth,0.5,bandwidth)){
  pi_cov=rep(c(pi_cov_,1-pi_cov_)/2,2)
  block_size_cov <- round(pi_cov * n)
  betahat=simulation_GRDPGwithCovariates(latent, beta, K, d, n, block_size, block_size_cov, cov, dmax)
  error=c(error,abs(betahat-beta))
}
plot(seq(0.1,0.5,0.025),error,xlab='Proportion of first covariate',ylab = 'beta error',main = 'n=2000, p=0.425, beta=0.15')
