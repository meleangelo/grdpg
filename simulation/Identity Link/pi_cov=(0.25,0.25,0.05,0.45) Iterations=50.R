...
simulation_GRDPGwithCovariates <- function(latent, beta, K, d, n, block_size, block_size_cov, cov, dmax, sd=FALSE, seed=2019) {
  ...
  ## Estiamte beta
  model <- Mclust(Xhat, 4, verbose = FALSE) 
...
for (i in seq(2,50)){
  seed=2019+i-1
  result=simulation_GRDPGwithCovariates(latent, beta, K, d, n, block_size,
                                        block_size_cov, cov, dmax,seed=seed)
  betahat=result$betahat
  phat=result$phat
  qhat=result$qhat
...
#plot(seq(3)*2,betaerror,xlab='Proportion of first covariate in second block',ylab = 'beta error',main = 'n=2000, p=0.35, beta=0.15')
...
pp1 <- ggplot(dat, aes(x=seq(50), y=betaerror)) + geom_line() + geom_point()
pp1 <- pp1 + labs(title = 'n=2000, p=0.35, beta=0.15,pi_cov0=c(0.25,0.25,0.05,0.45)', x = 'iterations', y = 'beta error')


dat=data.frame(perror)
pp2 <- ggplot(dat, aes(x=seq(50), y=perror)) + geom_line() + geom_point()
pp2 <- pp2 + labs(title = 'n=2000, p=0.35, beta=0.15, pi_cov0=c(0.25,0.25,0.05,0.45)', x = 'iterations', y = 'p error')


dat=data.frame(qerror)
pp3 <- ggplot(dat, aes(x=seq(50), y=qerror)) + geom_line() + geom_point()
pp3 <- pp3 + labs(title = 'n=2000, p=0.35, beta=0.15, pi_cov0=c(0.25,0.25,0.05,0.45)', x = 'iterations', y = 'q error')
